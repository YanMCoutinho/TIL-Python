...
class Usuario:
    ...
    def propoe_lance(self, leilao, valor):
        if not self._valor_for_valido(valor):
            raise LanceInvalido('Não pode propor um lance com um valor maior do que o valor da carteira')

        lance = Lance(self, valor)
        leilao.propoe(lance)
        self.__carteira -= valor

    def _valor_for_valido(self, valor):
